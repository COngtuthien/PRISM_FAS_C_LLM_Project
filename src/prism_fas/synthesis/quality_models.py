...
PINNED = {
    "detector": {"role": "detection_and_landmarks", "backend": "scrfd_10g_bnkps", "file": "scrfd_10g_bnkps.onnx",
                 "relative_path": "face_detectors/scrfd_10g_bnkps.onnx", "revision": None,
    ...
    "parsing": {"role": "parsing_and_pose", "backend": "facexformer", "file": "model.pt",
                "relative_path": "face_geometry/ckpts/model.pt", "repo_id": "kartiknarayan/facexformer",
...
def resolve_weight(weight_root: Path, role: str, *, verify: bool = True) -> Path:
    spec = PINNED[role]
    path = Path(weight_root) / spec["relative_path"]
    if not path.is_file(): raise QualityModelError(f"pinned {role} weight missing at <weight_root>/{spec['relative_path']}")
    if verify:
