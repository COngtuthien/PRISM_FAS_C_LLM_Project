...
class _StubTower:
    ...
    def __call__(self, *, pixel_values: Any) -> Any:
        ...
        # Drawn on the CPU under the fixed seed, then moved: a CUDA generator
        # under the same seed draws different numbers, and this stub must stay
        # bit-identical to the CPU audit it was written for. The move is what
        # keeps the stub from handing host tensors to a model on the GPU — the
        # region path concatenates these with device tensors and would otherwise
        # fail inside `region_embeddings` on any CUDA host.
        generator = torch.Generator().manual_seed(20260806)
        last = torch.rand(batch, self.tokens, self.dim, generator=generator).to(pixel_values.device)
