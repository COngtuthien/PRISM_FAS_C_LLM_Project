...
from prism_fas.data.preprocess_m2 import Detection, DetectorInferenceError, InvalidBoundingBoxError, select_largest
...
def _context_record(tmp_path):
    ...
    context = PreprocessingRunContext(project_root=tmp_path, work_root=tmp_path, run_profile="full_preprocessing", output_namespace="full_preprocessing", output_root=layout.output_root, crops_root=layout.crops_root, frames_root=layout.frames_root, manifests_root=layout.manifests_root, state_root=layout.state_root, reports_root=layout.reports_root, logs_root=layout.logs_root, run_id="invalid-bbox", dataset="casia_fasd", dataset_role="source", preprocessing_version="m2-v1", preprocessing_config_hash="a" * 64, detector_model_path=tmp_path / "model.onnx", detector_model_sha256="b" * 64, detector_input_size=320, detector_threshold=.5, all_records=False, record_limit=1, sample_limit=None, resume=False, dry_run=False, partial_full_profile=False, command="test")
...
def _reader(count=1):
    class Reader:
        ...
        def read_frame(self, index): return FrameResult(index, index + 7, None, 64, 64, np.zeros((64, 64, 3), dtype=np.uint8), "synthetic-reader")
        def close(self): pass
    return Reader()


@pytest.mark.parametrize("bbox", [(30, 10, 10, 40), (10, 30, 40, 10), (float("nan"), 10, 40, 40), (100, 100, 130, 130)])
def test_invalid_bbox_variants_raise_typed_geometry_error(bbox):
    detection = Detection(bbox=bbox, score=.91, landmarks=[(20, 22)] * 5)
    if bbox[0] == 100:
        assert select_largest([detection], .5, 16) is detection
    else:
        with pytest.raises(InvalidBoundingBoxError): select_largest([detection], .5, 16)


def test_source_invalid_bbox_routes_failure_before_counter_or_crop(tmp_path, monkeypatch):
    ...
    class Detector:
        def detect(self, image): return [Detection(bbox=(30, 10, 10, 40), score=.91, landmarks=[(20, 22)] * 5)]
    ...
    assert routed["requested_frame_index"] == 0 and routed["actual_frame_index"] == 7
    assert routed["stage"] == "geometry" and routed["error_code"] == "invalid_bbox" and routed["recoverable"] is True
    assert ":\\" not in routed["error_message"] and "30" not in routed["error_message"]
    ...
    assert result.failures_by_code == {"invalid_bbox": 1}
    assert not list(context.crops_root.rglob("*"))


def test_invalid_bbox_helper_exception_and_missing_repository_are_fatal(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(m2_runner, "route_preprocessing_failure", lambda **kwargs: (_ for _ in ()).throw(RuntimeError("synthetic bbox routing error")))
    with pytest.raises(RuntimeError, match="^synthetic bbox routing error$"):
...
def test_real_repository_invalid_bbox_persists_one_failure_row(tmp_path):
    context, record = _context_record(tmp_path)
    repository = ManifestRepository(context.manifests_root, _metadata(context))
    class Detector:
        def detect(self, image): return [Detection(bbox=(100, 100, 130, 130), score=.91, landmarks=[(20, 22)] * 5)]
    result = m2_runner.run_preprocessing(context, [record], detector=Detector(), media_reader_factory=lambda _: _reader(), repository_factory=lambda *_: repository)
    assert result.manifest_counts == {"source_frames": 0, "source_crops": 0, "target_frames": 0, "target_crops": 0, "preprocessing_failures": 1}
    repository.load()
    assert repository.rows["preprocessing_failures"][0]["error_code"] == "invalid_bbox"


def test_routed_invalid_bbox_continues_to_valid_source_success(tmp_path):
    context, record = _context_record(tmp_path)
    repository = ManifestRepository(context.manifests_root, _metadata(context))
    class Detector:
        ...
        def detect(self, image):
            self.calls += 1
            return [Detection(bbox=(30, 10, 10, 40), score=.91, landmarks=[(20, 22)] * 5)] if self.calls == 1 else [Detection(bbox=(12, 12, 52, 52), score=.91, landmarks=[(20, 22)] * 5)]
...
def test_non_geometry_results_do_not_route_invalid_bbox(tmp_path, monkeypatch, detector):
    ...
    monkeypatch.setattr(m2_runner, "route_preprocessing_failure", lambda **kwargs: routed.append(kwargs))
    result = m2_runner.run_preprocessing(context, [record], detector=detector, media_reader_factory=lambda _: _reader(), repository_factory=lambda *_: Repository())
    assert not any(call["error_code"] == "invalid_bbox" for call in routed)
    assert result.failures_by_code.get("invalid_bbox", 0) == 0
